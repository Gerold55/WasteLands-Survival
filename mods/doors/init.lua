...
function doors.door_toggle(pos, node, clicker)
	...
	if clicker and not ws_core.can_interact_with_node(clicker, pos) then
		return false
	end
...
function doors.register(name, def)
	...
	if not def.sounds then
		def.sounds = ws_core.node_sound_wood_defaults()
	end
...
doors.register("door_wood_screen", {
		tiles = {{ name = "doors_door_wood_screen.png", backface_culling = true }},
		description = S("Wooden Screen Door"),
		inventory_image = "doors_item_wood_screen.png",
		groups = {node = 1, choppy = 2, oddly_breakable_by_hand = 2, flammable = 2},
		recipe = {
			{"group:wood", "group:wood"},
