...
-- Cache/parse water level once
local WATER_LEVEL = tonumber(minetest.get_mapgen_setting("water_level") or "1") or 1

-- Cleanly remove counterpart nodes
local function remove_if_matches(pos, expected_name)
...
minetest.register_node("ws_core:sand_with_cattails", {
    ...
    paramtype = "light",
    paramtype2 = "leveled",  -- used by plantlike_rooted for stem height visuals
    ...
    selection_box = {
        type = "fixed",
        fixed = {
            {-0.5, -0.5, -0.5, 0.5, 0.5, 0.5},       -- shore block
            {-4/16, 0.5, -4/16, 4/16, 1.5, 4/16},   -- visible plant area
        },
    },

    -- Ensure the top exists when generated by mapgen/decorations
    on_construct = function(pos)
        local water_pos = {x = pos.x, y = pos.y + 1, z = pos.z}
        local top_pos   = {x = pos.x, y = pos.y + 2, z = pos.z}
        local n1 = minetest.get_node(water_pos)
        if is_water(n1.name) and is_buildable(top_pos) then
            local n2 = minetest.get_node(top_pos)
            if n2.name ~= "ws_core:cattail_top" then
    ...
    -- Placement rules:
    -- base_pos (y)     : ws_core:sandy_dirt
    -- water_pos (y+1)  : any water (group:water)
    -- top_pos (y+2)    : air (buildable_to)
    on_place = function(itemstack, placer, pointed_thing)
        ...
        -- A) Clicked the shore block directly
        if node_under.name == "ws_core:sandy_dirt" then
            base_pos = vector.new(under)

        -- B) Clicked the water surface: base is exactly 1 node below the water
        elseif is_water(node_under.name) then
            local below = {x = under.x, y = under.y - 1, z = under.z}  -- fixed (-1)
            local node_below = minetest.get_node(below)
            if node_below.name == "ws_core:sandy_dirt" then
                base_pos = below
            end
        end
        ...
        local water_pos = {x = base_pos.x, y = base_pos.y + 1, z = base_pos.z}
        local top_pos   = {x = base_pos.x, y = base_pos.y + 2, z = base_pos.z}

        local node_water = minetest.get_node(water_pos)
        local node_top   = minetest.get_node(top_pos)

        if not is_water(node_water.name) then
            return itemstack
        end

        local top_def = minetest.registered_nodes[node_top.name] or {}
        if not (node_top.name == "air" or top_def.buildable_to) then
            return itemstack
        end

        if minetest.is_protected(base_pos, player_name)
        or minetest.is_protected(water_pos, player_name)
        or minetest.is_protected(top_pos,   player_name) then
            minetest.chat_send_player(player_name, "Node is protected")
            minetest.record_protection_violation(base_pos, player_name)
            return itemstack
        end

        minetest.set_node(base_pos, { name = "ws_core:sand_with_cattails", param2 = 16 })
        minetest.set_node(top_pos,  { name = "ws_core:cattail_top" })
    ...
    -- If the base is destroyed, restore the shore and remove the top.
    after_destruct = function(pos, oldnode)
        minetest.set_node(pos, { name = "ws_core:sandy_dirt" })
        local top_pos = {x = pos.x, y = pos.y + 2, z = pos.z}
        remove_if_matches(top_pos, "ws_core:cattail_top")
    end,
})
...
minetest.register_node("ws_core:cattail_top", {
    ...
    -- If the top is removed, also remove the rooted base and restore shore.
    after_destruct = function(pos, oldnode)
        local base_pos = {x = pos.x, y = pos.y - 2, z = pos.z}
        local node = minetest.get_node_or_nil(base_pos)
        if node and node.name == "ws_core:sand_with_cattails" then
            minetest.remove_node(base_pos)
        end
    end,
})
